...
/* async function fetchData(code) {
  if (!code) return
  try {
    const res = await fetch(`http://localhost/apiLicence2025/controller/fournisseur/getOneFour.php?codefour=${code}&host=localhost&dbname=licence2025&username=root&password=`)
    const data = await res.json()
    console.log("Données chargées :", data.codefour)
    if (res.ok && data) {
      nomfour.value = data.nomfour
      prenomfour.value = data.prenomfour
      tel1four.value = data.tel1four
      tel2four.value = data.tel2four
      adressefour.value = data.adressefour
      cafour.value = data.cafour
      soldeinitfour.value = data.soldeinitfour

    } else {
      error.value = "Erreur de chargement du fournisseur"
    }
  } catch (e) {
    error.value = "Erreur réseau"
  }
} */
// Charger dès que codefour change
watch(() => props.fournisseur, (f) => {
  if (f) {
    codefour.value = f.codefour
    nomfour.value = f.nomfour
    prenomfour.value = f.prenomfour
    tel1four.value = f.tel1four
    tel2four.value = f.tel2four
    adressefour.value = f.adressefour
    cafour.value = f.cafour
    soldeinitfour.value = f.soldeinitfour
    soldefour.value = f.soldefour
  }
}, { immediate: true })
...
async function handleSubmit() {
  ...
  try {
    const res = await fetch('http://localhost/apiLicence2025/controller/fournisseur/updatefournisseur.php?host=localhost&dbname=licence2025&username=root&password=', {
    ...
    if (res.ok) {
      success.value = data.message || "Fournisseur Modifier avec succès."
