...
//chargement des champs losque le matricule change
watch(() =>props.personnel,(p) =>{
    if(p){
        matriculePers.value=p.matriculePers
        nompers.value=p.nompers
        prenompers.value=p.prenompers
        datenaispers.value=p.datenaispers
        lieunaispers.value=p.lieunaispers
        numtelpers.value=p.numtelpers
        numcnipers.value=p.numcnipers
        datevalidite.value=p.datevalidite
        sexepers.value=p.sexepers
        statutpers.value=p.statutpers
        idfonc.value=p.idfonc
        idserv.value=p.idserv
    }
},{immediate: true})
const modifierPersonnel = async () => {
  const data = {
  ...
  try {
    const res = await fetch('http://localhost/apiLicence2025/controller/personnel/updatepersonnel.php?host=localhost&dbname=licence2025&username=root&password=', {
      method: 'POST',
    ...
    message.value = result.message || 'Personnel modifié'
    resetForm()
  } catch (err) {
...
<template>
  <div class="container mt-4">
    <div class="card shadow">
      <div class="card-body">
        <form @submit.prevent="modifierPersonnel">
          <div class="row g-3">
            <div class="col-md-4">
